...
abstract class IScanRemoteDatasource {
  ...
  Future<Either<Failure, ScanResponseModel>> objectScannedV5(
      XFile photo, String accessToken,
      {String? email});
  Future<Either<Failure, ScanResponseModel>> objectScannedV6(
      XFile photo, String accessToken,
      {String? email});
...
class ScanRemoteDatasource extends IScanRemoteDatasource {
  ...
  Future<Either<Failure, InventoryItemModel>> getInventoryItemByID(
      String id) async {
    try {
      var reponseResult = await _helperUtil.getRequest(
          dio, "$inventoryUrl/api/inventory/$id/info");
