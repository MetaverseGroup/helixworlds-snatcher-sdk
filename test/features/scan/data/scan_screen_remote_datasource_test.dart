...
main() {
  ...
  group("mapping InventoryItemModel test", () {
    test("success", () async {
      try {
        ...
        var result = InventoryItemModel.fromJson(jsonDecode(jsonData));
  ...
  group("testing getInventoryItemByID", () {
    test("success mock 1ee status code 200", () async {
      var jsonData = await scanSuccessResponse;
      when(remoteDS?.getInventoryItemByID("1ee")).thenAnswer(
        (_) async {
          return Right(InventoryItemModel.fromJson(jsonDecode(jsonData)));
        },
      );
    ...
    test("failure mock 1234 status code 404", () async {
      when(remoteDS?.getInventoryItemByID("1234")).thenAnswer(
        (_) async {
